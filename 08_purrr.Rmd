...
- Write a *named function* for performing z-normalization [1] that takes a vector `.x` as input.
...
- Draw some from a Gaussian using `rnorm(n, mean, sd)` and store them as double vectors in a list.
...
- Perform the z-normalization using a `for`-loop.
...
- Perform the z-normalization using `map()` but use an *anonymous function*.
...
- Perform the z-normalization using `map()` but use an *`purrr`-style function*.
