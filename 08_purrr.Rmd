...
Second, we draw samples from the `penguin` data set and store them as double vectors in a list.
```{r}
samples <- list(
...
???
- pull vector out, and pluck element out
...
nested_penguins <- nested_penguins %>% 
  dplyr::mutate(coefs = map(
    .x = lin_reg,
    .f = ~ { summary(.x) %>% .$coefficients %>% as_tibble }
  )) %>%
  dplyr::select(-data, -lin_reg)

nested_penguins
```

.footnote[.pull-right[.pull-right[
*Note: Here we dropped the `lin_reg` and `data` columns, otherwise we would have carried around a lot of redundant data which may exceed our memory storage capacity very quickly.*
]]]
