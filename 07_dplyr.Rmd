...
Filter for all penguins where the a missing value in the `bill_length_mm` measurement:
```{r, eval=F}
penguins %>% 
...
*But never forget to ungroup your data, otherwise you may run into errors later on!*
...
**Stacked `group_by()`:** If `.add = T` is omitted, the second `gruop_by()`overrides the first.
```{r, eval=F}
penguins %>% 
  group_by(species) %>% 
...
**Nesting of grouped data:** Usually, you will find it more intuitiv to use `group_by()` followed by `nest()` to produce a nested data frame compared to example on [slide 27](#tidyr_nest).
```{r, eval=F}
penguins %>% 
