...
Pick the first `n` rows in a data frame (vice versa for `slice_tail()`):
```{r}
penguins %>% 
...
Pick the `n` rows with the largest value in a data frame (vice versa for `slice_min()`):
```{r}
penguins %>% 
...
penguins %>% 
  arrange(body_mass_g) %>% 
  slice_head(n = 3)  # equivalent to: slice_min(body_mass_g, n = 3)
```
...
penguins %>% 
  ...
  slice_head(n = 3)  # equivalent to: slice_max(body_mass_g, n = 3)
```
...
Which columns are returned by the following queries? Try it out at home `r emo::ji("smiling_face")`
...
```{r}
penguins %>% 
  mutate(bm_kg = body_mass_g / 1000, .keep = "all", .before = body_mass_g) %>% 
  slice_head(n = 5)
```
...
**`lag()`** shifts column values by an offset `n` forward
```{r}
...
**`lead()`** shifts column values by an offset `n` backward
```{r}
penguins %>% select(species, body_mass_g) %>% 
  mutate(lead_bm = lead(body_mass_g, n = 2))
```
]
.panel[.panel-name[*_join()]
...
???
src: https://www.rstudio.com/resources/rstudioglobal-2021/the-dynamic-duo-sql-and-r/
