...
`tidyr` provides several functions that help you bring your data into *tidy data* format (cf. [tidy data](#tidy data)), i.e. change the structure of your data set.
...
1) “**Pivotting**” which converts between long and wide forms using `pivot_longer()` and `pivot_wider()`:
...
2) "**Nesting**" which groups similar data such that each group becomes a single row in a data frame (the structure is then referred to as *nested data frame*).
...
3) “**Rectangling**” which disentangles nested data structures (e.g., JSON, HTML) and brings it into *tidy* format (see `unnest_longer()`, `unnest_wider()`).

4) "**Splitting**" and "**combining**" to transform a single character column into multiple columns and vice versa.
...
5) "**Handling missing values**" by making implicit `NA` explicit (`complete()`), by making explicit `NA` implicit (`drop_na()`) or by replacing `NA` with the next/previous value (`fill()`) or a known value (`replace_na()`).
.pull-left[
```{r, echo=F}
